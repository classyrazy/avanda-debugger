<template>
	<transition appear name="toast">
		<div v-if="active" :class="[
			`overflow-hidden max-w-[90%]  shadow-xl fixed top-4 right-4  box border-[1.5px] py-4 px-2 rounded-md box border-dark  bg-white z-[900000000000000]`,
		]">
			<div :style="`width:${BORDER_WIDTH}%`"
				:class="[`${BG} h-1  absolute  bottom-0 left-0 transition-all duration-300`]" />
			<span @click="closeAlert()">
				<close-icon
					class="text-primary w-5 h-5 absolute right-2 top-2 cursor-pointer rounded-md my-auto"
					@click="closeAlert()"></close-icon>
			</span>

			<div class="flex items-center pr-12 gap-3">
				<Success :class="[`${TEXT} w-5 h-5`]" v-if="type === 'SUCCESS'" />
				<Error :class="[`${TEXT} w-5 shrink-0`]" v-if="type === 'ERROR'" />
				<Warning :class="[`${TEXT} w-5`]" v-if="type === 'Alert'" />
				<div>
