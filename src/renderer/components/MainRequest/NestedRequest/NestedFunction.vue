<template>
    <div class="border w-[70%] my-4 rounded-md  mx-8" :class="nestedFunctionOpen ? 'pb-10' : ''">
        <div class="title-input-and-dropdown-icon flex justify-between  px-4">
            <div class="grid grid-cols-8">
                <div class="border-r w-full col-span-3">
                    <input placeholder="Name e.g blog"
                        ...
                        type="text" v-model="item.name" @keyup="emit('updateModel', modelValues)">
                </div>
                <div class="w-full col-span-5">
                    <input placeholder="Nexted request's Controller/ServiceName"
                        ...
                        type="text" v-model="item.service" @keyup="emit('updateModel', modelValues)">
                </div>
            </div>
            <div class="flex gap-2   items-center ">
                ...
                <div class="cursor-pointer w-7 h-7 p-1 hover:bg-gray-300 rounded-full flex justify-center items-center"
                    title="Create a new nested Function" @click="deleteNestedFunction(item.id)">
    ...
    <div class="ml-4" v-if="item.nestedFunction">
        <nested-function v-for="(nestedItem, idx) in item.nestedFunction" :key="idx" :item="nestedItem ?? {}" :currId="idx"
            @update-model="handleEditNestedFunction($event, idx)">{{ idx }}</nested-function>
...
import ColumnEditSmall from '../../MainRequest/InnerTabs/Column/ColumnEditSmall.vue'
import ParamEditSmall from '../../MainRequest/InnerTabs/Param/ParamEditSmall.vue'
import { nestedFunctionType } from '../../../types/nestedFunctionType';
import { computed, ref } from 'vue';
import { defineProps, defineEmits } from 'vue'
import { createNewNestedFunction, deleteNestedFunction } from '../../../composables/request/nestedRequest';
